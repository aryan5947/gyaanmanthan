...
interface CommentsModalProps {
  postMetaId: string;   // ✅ updated
  postMetaAuthorId: string; // ✅ Add this line
  isOpen: boolean;
  onClose: () => void;
}
...
const PostMetaCommentsModal: React.FC<CommentsModalProps> = ({
  postMetaId,
  postMetaAuthorId,
  isOpen,
  onClose,
  ...
  async function fetchComments() {
    try {
      const res = await fetch(`${API_BASE}/postMeta-comments/${postMetaId}`);
    ...
    try {
      const res = await fetch(`${API_BASE}/postMeta-comments/${postMetaId}`, {
       ...
       body: JSON.stringify({
           ...
           postMetaAuthorId, // ✅ Include this in request body
         }),
      });
  ...
  async function handleAddReply(commentId: string) {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/postMeta-comments/${commentId}/reply`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async function handleToggleLike(commentId: string) {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/postMeta-comments/${commentId}/like`,
        {
          method: "POST",
  ...
  async function handleToggleReplyLike(commentId: string, replyId: string) {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/postMeta-comments/${commentId}/reply/${replyId}/like`,
        {
          method: "POST",
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  async function handleDelete(commentId: string) {
    ...
    try {
      const res = await fetch(`${API_BASE}/postMeta-comments/${commentId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
  ...
  async function handleDeleteReply(commentId: string, replyId: string) {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/postMeta-comments/${commentId}/reply/${replyId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
...
export default PostMetaCommentsModal;
